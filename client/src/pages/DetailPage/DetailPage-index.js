...
function DetailPage({ match }) {
    ...
    useEffect(() => {
        ...
        window.scrollTo(0, 0);
        axios.get(`http://api.visitkorea.or.kr/openapi/service/rest/KorService/detailCommon?serviceKey=W%2B9SSMZKirmvzGAhoiAssJvfD9PYntkIlyMO6niTjMi5nwx%2BrUP%2FiQqrLccqOmuxPU2w7iyFQkUC41ddNiePUg%3D%3D`,
        {params:{
          MobileOS:'ETC',
    ...
    const overView = overview.replace(/(<([^>]+)>)/ig,"");
    ...
    return (
      <>
        ...
        <Styled.Div>
            ...
            {pageURL ? <Styled.PageURL href = {pageURL} target="_blank" title = {`새창 : ${title}`}>홈페이지로 이동</Styled.PageURL>:null}
            {/* {pageURL} */}
            {imgURL ? <Styled.Img src= {imgURL} /> : <Styled.Img src= {notImageYet} />}    
            
            <Styled.Overview>
              <span className = 'first-overview'>{FirstOverView}</span>
              <Styled.ReadMoreBtn className = {!readMore ? null : 'hide'} onClick = {readMoreHandler}>...더보기</Styled.ReadMoreBtn>
              <span className = {readMore ? null : 'hide'}>{SecondOverView}</span>
              <Styled.CutDownBtn className = {readMore ? null : 'hide'} onClick = {readMoreHandler}>...간략히</Styled.CutDownBtn>
            </Styled.Overview>              
            <MapInRoom />
