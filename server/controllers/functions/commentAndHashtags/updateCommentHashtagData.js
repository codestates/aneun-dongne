...
module.exports = async (comment_id, comment_user_id, comment_post_contentid, comment_content, comment_tags_arr) => {
  ...
  await comment.setHashtags(null); //기존 해시태그들과의 연결을 끊는다
  //   let result = await comment.getHashtags({ raw: true });

  if (comment_tags_arr.length > 0 || comment_tags_arr[0] !== "") {
    // 해시태그를 하나라도 달았을 경우 //태그를 안 달면 [] 이렇게 온다
